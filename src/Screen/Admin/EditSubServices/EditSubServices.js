...
const EditSubServices = ({ navigation, route }) => {
  const [id, setId] = useState(route?.params?.Item?.Id);

  const [Name, setName] = useState(route?.params?.Item?.SubServicesName);
  const [img, setImg] = useState(route?.params?.Item?.SubServicesImage);
  const [price, setPrice] = useState(route?.params?.Item?.Price)
  const [loading, setLoading] = useState(false);
  const [disable1, setDisable1] = useState(true);

  console.log("route.params", route?.params?.Item?.Id)
  ...
  const addService = async () => {
    ...
    if (Name === " ") {
      ...
      const baseUrl = `${IP}UpdateSubServices`;
      console.log(baseUrl);
      ...
      try {
        const response = await axios.post(baseUrl, {
          ...
          SubServiceImage: img,
          Price: price,
          ServicesId: route?.params?.Item?.ServicesId
        ...
        if (response?.status === 200) {
          ...
          setPrice('');
          setImg('');
          var temp = { ...route?.params?.Item, ServiceName: route?.params?.ServiceName }
          console.log("temp", temp)
          navigation.navigate('ShowSubServices', { Item: temp })
